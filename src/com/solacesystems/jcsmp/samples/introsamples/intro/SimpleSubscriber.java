...
public class SimpleSubscriber {
    ...
    public static void main(String... args) throws JCSMPException {
        if (args.length < 4) {
            System.out
                    .println("Usage: SimpleSubscriber <msg_backbone_ip:port> <vpn> <client-username> <password> <topic> ");
            System.out
                    .println("        - to use wildcard for topic, that argument must be enclosed by quotes");
            System.exit(-1);
        }
        System.out.println("SimpleSubscriber initializing...");
        final JCSMPProperties properties = new JCSMPProperties();
        properties.setProperty(JCSMPProperties.HOST, args[0]);  	// msg-backbone-ip:port
        properties.setProperty(JCSMPProperties.VPN_NAME, args[1]); 	// message-vpn
        properties.setProperty(JCSMPProperties.USERNAME, args[2]);	// client-username 
        properties.setProperty(JCSMPProperties.PASSWORD, args[3]);	// password 
        ...
        System.out.println("Connected. Awaiting message... Press <ENTER> to stop");
        cons.start();
